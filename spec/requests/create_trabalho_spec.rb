...
RSpec.describe 'Aluno cria novo trabalho', type: :request do
  ...
  context 'success 201' do
    it 'responde com o id do trabalho' do
      post "/api/v1/trabalhos", params: { trabalho: { title: 'NAC', url: 'localhost', aluno_id: aluno.id } }
  ...
  context 'failure 422' do
    it "raises error if no title is suplied" do
      post "/api/v1/trabalhos", params: { trabalho: { url: 'localhost', aluno_id: aluno.id } }

      expect(response).to have_http_status(:unprocessable_entity)
    end

    it "raises error if no url is suplied" do
      post "/api/v1/trabalhos", params: { trabalho: { title: 'NAC', aluno_id: aluno.id } }

      expect(response).to have_http_status(:unprocessable_entity)
    end

    it "raises error if no student is suplied" do
      post "/api/v1/trabalhos", params: { trabalho: { title: 'NAC', url: 'localhost' } }
